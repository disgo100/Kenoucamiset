...
function configurarSistemaAutenticacion() {
  ...
  if (btnTogglePass && inputPassword && iconTogglePass) {
    btnTogglePass.addEventListener("click", () => {
      ...
      iconTogglePass.className = isPassword
        ? "fa-solid fa-eye-slash"
        : "fa-solid fa-eye";
      btnTogglePass.style.color = isPassword
        ? "var(--accent)"
        : "var(--text-muted)";
...
function verificarEstadoSeguridadPrevio() {
  const estaBloqueadoPermanente =
    localStorage.getItem("admin_user_permanently_blocked") === "true";
  if (estaBloqueadoPermanente) {
    mostrarPantallaUsuarioBloqueado();
    return;
  }
  const lockoutUntil = parseInt(
    localStorage.getItem("admin_lockout_until") || "0",
    10,
  );
    ...
    alertBox.innerHTML = `
            ...
            <div style="border-top: 1px dashed rgba(255,51,102,0.4); padding-top: 0.8rem; margin-top: 0.8rem;">
                <div style="display: flex; flex-direction: column; gap: 0.6rem;">
                    <button type="button" onclick="reiniciarSeguridadAdmin()" style="width: 100%; padding: 0.7rem; background: var(--accent); color: #fff; border: none; border-radius: 8px; font-weight: 700; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 0.5rem; transition: opacity 0.2s;">
                        <i class="fa-solid fa-rotate-left"></i> Restablecer Bloqueo / Reintentar Acceso
                    </button>
...
window.reiniciarSeguridadAdmin = function () {
  ...
  if (trigger && fileInput) {
    ...
    fileInput.addEventListener("change", (e) => {
      ...
      if (!["image/jpeg", "image/jpg", "image/png"].includes(file.type)) {
        alert(
          "⚠️ Formato de archivo no válido. Seleccione un archivo JPG, JPEG o PNG.",
        );
        return;
      }
...
function obtenerProductosAdmin() {
  ...
  const tieneMangaSisa =
    parsed &&
    parsed.some((p) => (p.nombre || "").toLowerCase().includes("manga sisa"));
  if (!parsed || parsed.length === 0 || !tieneMangaSisa) {
    localStorage.setItem(
      "prendas_admin_products",
      JSON.stringify(PRODUCTOS_INICIALES_CATALOGO),
    );
    localStorage.setItem(
      "prendas_catalog",
      JSON.stringify(PRODUCTOS_INICIALES_CATALOGO),
...
function configurarGestionProductos() {
  ...
  const abrirModal = () => {
    document.getElementById("modal-prod-title").innerHTML =
      '<i class="fa-solid fa-shirt" style="color: var(--accent);"></i> Cargar / Editar Prenda';
  ...
  if (formModal) {
    formModal.addEventListener("submit", (e) => {
      ...
      const precio =
        parseFloat(document.getElementById("prod-precio").value) || 0;
      const stock =
        parseInt(document.getElementById("prod-stock").value, 10) || 0;
      ...
      if (!imagenBase64) {
        if (categoria === "Camisetas")
          ...
          imagenBase64 =
            "https://images.unsplash.com/photo-1588850561407-ed78c282e89b?w=500";
        else if (categoria === "Busos" || categoria === "Hoodies")
          imagenBase64 =
            "https://images.unsplash.com/photo-1556905055-8f358a7a47b2?w=500";
        else if (categoria === "Pantalones" || categoria === "Joggers")
          imagenBase64 =
            "https://images.unsplash.com/photo-1624378439575-d8705ad7ae80?w=500";
        else
          imagenBase64 =
...
function editarProducto(id) {
  ...
  if (!prod) return;
  document.getElementById("modal-prod-title").innerHTML =
    '<i class="fa-solid fa-pen-to-square" style="color: #00f2fe;"></i> Editar Prenda';
...
const PEDIDOS_MOCK_INICIALES = [
  {
    orden: "PS-849201",
    cliente: "Camilo Gómez",
    contacto: "3001234567",
    detalle: "2x Camiseta Oversize + Estampado DTF",
    total: 128000,
    estado: "En proceso",
  },
  {
  ...
  },
  {
    ...
    cliente: "Diego Lemus",
    contacto: "3157774411",
    detalle: "3x Gorra Trucker Snapback",
    total: 96000,
    estado: "Vendido",
  },
  {
    orden: "PS-449102",
    cliente: "Laura Restrepo",
    contacto: "3189990011",
    detalle: "2x Short Deportivo Streetwear",
    total: 76000,
    estado: "Entregado",
  },
  {
    ...
    contacto: "3012223344",
    detalle: "1x Jogger Cargo + Estampado Vinilo",
    total: 83000,
    estado: "Finalizado",
  },
];
function obtenerPedidosAdmin() {
  ...
  if (pedsCart) {
    try {
      combinados = combinados.concat(JSON.parse(pedsCart));
    } catch (e) {}
  }
  ...
  if (pedsAdmin) {
    ...
    localStorage.setItem(
      "prendas_admin_orders",
      JSON.stringify(PEDIDOS_MOCK_INICIALES),
    );
...
function configurarGestionPedidos() {
  ...
  if (btnVaciar) {
    btnVaciar.addEventListener("click", () => {
      if (
        confirm(
          "🗑️ ¿Desea vaciar el historial completo de pedidos registrados?",
        )
      ) {
...
function renderizarTablaPedidos() {
  ...
  const totalEnt = peds.filter(
    (p) => p.estado === "Entregado" || p.estado === "Finalizado",
  ).length;
  ...
  if (busquedaPedidoActual) {
    filtrados = filtrados.filter(
      (p) =>
        ...
        (p.contacto &&
          p.contacto.toLowerCase().includes(busquedaPedidoActual)) ||
...
function configurarVentanaInventarioMetricas() {
  ...
  const btnCerrarDetalleAccion = document.getElementById(
    "btn-cerrar-modal-detalle-btn",
  );
  ...
  if (btnCloseDetalle && modalDetalle) {
    btnCloseDetalle.addEventListener(
      "click",
  ...
  if (btnCerrarDetalleAccion && modalDetalle) {
    btnCerrarDetalleAccion.addEventListener(
      "click",
      () => (modalDetalle.style.display = "none"),
    );
  }
...
function renderizarModalInventario() {
  ...
  const totalValorizado = prods.reduce(
  ...
  if (elValor)
    elValor.textContent = `$${totalValorizado.toLocaleString("es-CO")}`;
  if (elAlertas) elAlertas.textContent = totalAlertas.toString();
  if (elVentas)
    elVentas.textContent = `$${totalVentas.toLocaleString("es-CO")}`;
  if (elOrdenes) elOrdenes.textContent = totalOrdenes.toString();
  if (elTimestamp)
    elTimestamp.textContent = `Sincronizado: ${new Date().toLocaleString("es-CO")}`;
...
function exportarInventarioPDF() {
  if (!window.jspdf) {
    alert(
      "❌ Error: La librería jsPDF no está disponible. Verifique su conexión.",
    );
    return;
  }
  ...
  doc.text(
    "Informe Oficial de Control de Inventario & Métricas Sincronizadas",
    14,
    26,
  );
  ...
  doc.text(
    `• Valorización de Inventario: $${totalValor.toLocaleString("es-CO")} COP`,
    125,
    y + 15,
  );
  doc.text(
    `• Ventas Totales Registradas: $${totalVentas.toLocaleString("es-CO")} COP`,
    18,
    y + 21,
  );
  ...
  doc.setTextColor(2, 132, 199);
  doc.text(
    `TOTAL INVENTARIO VALORIZADO: $${totalValor.toLocaleString("es-CO")} COP`,
    90,
    y,
  );
  ...
  doc.text(
    "Kenou Camisetas — Sistema Administrativo B2C | Documento Oficial de Auditoría de Inventario",
    14,
    287,
  );
...
function exportarPedidosPDF() {
  ...
  doc.setFontSize(9);
  doc.text(
    `TOTAL RECAUDADO EN ÓRDENES: $${totalVentas.toLocaleString("es-CO")} COP`,
    18,
    y + 10,
  );
  ...
  setTimeout(() => {
    doc.save(
      `Reporte_Pedidos_PrendasStylos_${now.toISOString().slice(0, 10)}.pdf`,
    );
  }, 100);
}
...
function exportarPedidosCSV() {
  ...
  let csvContent =
    "data:text/csv;charset=utf-8,Orden,Cliente,Contacto,Detalle,Total_COP,Estado\n";
  peds.forEach((p) => {
  ...
  link.setAttribute(
    "download",
    `Reporte_Pedidos_PrendasStylos_${new Date().toISOString().slice(0, 10)}.csv`,
  );
